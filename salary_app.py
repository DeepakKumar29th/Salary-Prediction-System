...
st.markdown("""
<style>
  /* Hide Streamlit anchor links on all headings */
  ...
  .result-box h1 { font-size: 2.8rem; margin: 0; color: white; }
...
with col1:
    edu_group = st.selectbox(
        "Degree Type",
        [PLACEHOLDER] + list(EDUCATION_DEGREES.keys())
    )

    if edu_group != PLACEHOLDER:
        edu_degree = st.selectbox(
            "Education Degree",
            [PLACEHOLDER] + EDUCATION_DEGREES[edu_group]
        )
    else:
        edu_degree = PLACEHOLDER
        st.selectbox("Education Degree", [PLACEHOLDER], disabled=True)

    if edu_degree != PLACEHOLDER:
        edu_level    = EDU_LEVEL[edu_degree]
        spec_options = SPECIALIZATIONS.get(edu_degree, ["General"])
        specialization = st.selectbox("Specialization", [PLACEHOLDER] + spec_options)
    else:
        specialization = PLACEHOLDER
        st.selectbox("Specialization", [PLACEHOLDER], disabled=True)

    experience = st.slider("Years of Experience", 0.0, 30.0, 0.0, 0.5)

with col2:
    sector = st.selectbox("Job Sector", [PLACEHOLDER] + list(SECTORS_ROLES.keys()))

    if sector != PLACEHOLDER:
        job_role = st.selectbox("Job Role", [PLACEHOLDER] + SECTORS_ROLES[sector])
    else:
        job_role = PLACEHOLDER
        st.selectbox("Job Role", [PLACEHOLDER], disabled=True)

    company_size = st.selectbox("Company Size", [PLACEHOLDER] + COMPANY_SIZES)
    city_tier    = st.selectbox(
        "Work City Tier",
        [PLACEHOLDER] + CITY_TIERS,
...
required = {
    "Degree Type":      edu_group,
    "Education Degree": edu_degree,
    "Specialization":   specialization,
    "Job Sector":       sector,
    "Job Role":         job_role,
    "Company Size":     company_size,
    "Work City Tier":   city_tier,
}
all_filled = all(v != PLACEHOLDER for v in required.values())

if st.button("Predict Salary", type="primary", use_container_width=True):
    if not all_filled:
        missing = [k for k, v in required.items() if v == PLACEHOLDER]
        ...
        <div class="result-box">
          <p style="font-size:0.95rem; margin-bottom:6px;">Predicted Annual Salary (CTC)</p>
        ...
        st.markdown("")
        st.metric("Monthly Take-Home (Approx)", f"Rs. {prediction/12/1e3:.1f}K")
